...
#pragma warning restore S2344
{
    ...
    /// the caller must be running as "LocalSystem" or "TrustedInstaller"; otherwise the system ignores this flag.
    /// For more information, see "You can circumvent AppLocker rules by using an Office macro on a computer
    /// that is running Windows 7 or Windows Server 2008 R2" in the Help
    /// and Support Knowledge Base at http://support.microsoft.com/kb/2532445.
    ...
    /// with exclusive write access for the calling process. Other processes cannot open the DLL file
    ...
    /// <summary>
    /// If this value is used, the system maps the file into the process's virtual address space
    /// as an image file. However, the loader does not load the static imports or perform
    /// the other usual initialization steps. Use this flag when you want to load a DLL only
    /// to extract messages or resources from it. If forced integrity checking is desired
    /// for the loaded file then LOAD_LIBRARY_AS_IMAGE is recommended instead.
    /// Unless the application depends on the image layout, this value should be used
    ...
    /// This value cannot be combined with LOAD_WITH_ALTERED_SEARCH_PATH.
    /// For example, if Lib2.dll is a dependency of C:\Dir1\Lib1.dll, loading Lib1.dll
    /// with this value causes the system to search for Lib2.dll only in C:\Dir1.
    /// To search for Lib2.dll in C:\Dir1 and all of the directories in the DLL search path,
    /// combine this value with LOAD_LIBRARY_DEFAULT_DIRS.
    /// Windows 7, Windows Server 2008 R2, Windows Vista, and Windows Server 2008:
    /// This value requires KB2533623 to be installed.
    /// Windows Server 2003 and Windows XP:  This value is not supported.
    /// </summary>
